...
const api = axios.create({
  baseURL: SERVER_URL,
  timeout: 60000, // 60 seconds timeout for general requests
  headers: {
    "Content-Type": "application/json",
  },
});

// Extended timeout for file processing operations
const FILE_PROCESSING_TIMEOUT = 120000; // 2 minutes for file processing
...
export const uploadFileAndChat = async (
  ...
  try {
    ...
    const response = await api.post("/upload-and-chat", formData, {
      ...
      timeout: FILE_PROCESSING_TIMEOUT, // Use extended timeout for file processing
    });
